...
class IntervalAroundtheCurrentPriceDemo(zs.Strategy):
    ...
    def next(self):
        if self.lines.update_timestamps[0]:
            ...
            if self.broker.getAsset().base_token_amt > 0:  # 还剩更多的u
                self.my_b_position = self.add_liquidity(current_price - self.a, current_price)
            else:
                self.my_b_position = self.add_liquidity(current_price, current_price + self.a)
...
myStrategy = IntervalAroundtheCurrentPriceDemo(a=100,b=10)
