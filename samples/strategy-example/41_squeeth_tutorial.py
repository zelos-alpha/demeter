"""
Squeeth tutorial — DISABLED (dead code)

This sample is disabled because:
  - ``MarketTypeEnum.squeeth`` has been commented out of the enum
  - ``demeter.squeeth`` module no longer exists in the codebase

The original code is preserved below as comments for reference.
If squeeth support is re-implemented in the future, this file can be
reactivated by uncommenting the relevant enum value and re-creating the
``demeter.squeeth`` module.
"""
