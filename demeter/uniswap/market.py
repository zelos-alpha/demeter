...
class UniLpMarket(Market):
    ...
    def __init__(
        self, market_info: MarketInfo, pool_info: UniV3Pool, data: pd.DataFrame = None
    ):
        """
        init UniLpMarket
    ...
    def get_price_from_data(self) -> pd.DataFrame:
        ...
        df = pd.DataFrame(
            index=price_series.index, data={self.quote_token.name: price_series}
        )
    ...
    def check_asset(self):
        ...
        if self.base_token not in self.broker.assets:
            raise DemeterError(
                f"base token {self.base_token.name} not exist in asset dict"
            )
        if self.quote_token not in self.broker.assets:
            raise DemeterError(
                f"quote token {self.quote_token.name} not exist in asset dict"
            )
    ...
    def __update_fee(self):
        ...
        for position_info, position in self._positions.items():
            V3CoreLib.update_fee(
                self.pool_info, position_info, position, self.market_status
            )

    def get_market_balance(
        self, prices: pd.Series | Dict[str, Decimal] = None
    ) -> MarketBalance:
        """
        ...
        net_value = (base_fee_sum + base_deposit_amount) * prices[
    ...
    def __remove_liquidity(
        ...
        sqrt_price_x96 = (
            int(sqrt_price_x96)
            if sqrt_price_x96 != -1
            else get_sqrt_ratio_at_tick(self.market_status.current_tick)
        )
        delta_liquidity = (
            liquidity
            if (liquidity is not None)
            and liquidity < self.positions[position].liquidity
        ...
        self._positions[position].liquidity = (
            self.positions[position].liquidity - delta_liquidity
        )
    ...
    def __collect_fee(
        ...
        token0_fee = (
            max_collect_amount0
            if max_collect_amount0 is not None
            and max_collect_amount0 < position.pending_amount0
            else position.pending_amount0
        )
        token1_fee = (
            max_collect_amount1
            if max_collect_amount1 is not None
            and max_collect_amount1 < position.pending_amount1
    ...
    def add_liquidity(
        ...
        self.record_action(
            AddLiquidityAction(
                ...
                base_balance_after=self.broker.get_token_balance_with_unit(
                    self.base_token
                ),
                quote_balance_after=self.broker.get_token_balance_with_unit(
                    self.quote_token
                ),
    ...
    def collect_fee(
        ...
        token0_get, token1_get = self.__collect_fee(
            self._positions[position], max_collect_amount0, max_collect_amount1
        )
    ...
    @float_param_formatter
    def buy(
        self, amount: Decimal | float, price: Decimal | float = None
    ...
    def even_rebalance(self, price: Decimal = None) -> (Decimal, Decimal, Decimal):
        ...
        total_capital = (
            self.broker.get_token_balance(self.base_token)
            + self.broker.get_token_balance(self.quote_token) * price
        )
        target_base_amount = total_capital / 2
        quote_amount_diff = target_base_amount / price - self.broker.get_token_balance(
            self.quote_token
        )
    ...
    def add_statistic_column(self, df: pd.DataFrame):
        ...
        * volume1: swap volume for token 1
        ...
        high_name, low_name = (
            ...
            if self.pool_info.is_token0_base
    ...
    def load_data(
        self, chain: str, contract_addr: str, start_date: date, end_date: date
    ):
        """
        ...
        * set timestamp as index
        ...
        full_indexes = pd.date_range(
            start=df.index[0], end=df.index[df.index.size - 1], freq="1min"
        )
