...
class V3CoreLib(object):
    ...
    @staticmethod
    def close_position(
        pool: UniV3Pool, position_info: PositionInfo, liquidity, sqrt_price_x96
    ):
        """
        close position
        :param pool: operation on which pool
        ...
        :return: token amount
        """
        return V3CoreLib.get_token_amounts(
            pool, position_info, sqrt_price_x96, liquidity
        )

    @staticmethod
    def get_token_amounts(
        pool: UniV3Pool, pos: PositionInfo, sqrt_price_x96, liquidity
    ) -> (Decimal, Decimal):
        """
    ...
    @staticmethod
    def quote_price_pair_to_tick(
        pool: UniV3Pool, lower_quote_price: Decimal, upper_quote_price: Decimal
    ):
        """
    ...
    @staticmethod
    def update_fee(
        pool: UniV3Pool, pos: PositionInfo, position: Position, state: UniV3PoolStatus
    ):
        """
        update fee
        :param pool: operation on which pool
        ...
        def calc_amounts():
            ...
            position.pending_amount0 += (
                from_wei(state.in_amount0, pool.token0.decimal) * share * pool.fee_rate
            )
        ...
        if state.last_tick:
            condition_over_position = (
                state.last_tick > pos.upper_tick and state.current_tick < pos.lower_tick
            ) or (
                state.current_tick > pos.upper_tick and state.last_tick < pos.lower_tick
            )
            condition_in_to_out_position = (
                pos.upper_tick >= state.last_tick >= pos.lower_tick
                and (
                    state.current_tick > pos.upper_tick
                    or state.current_tick < pos.lower_tick
                )
            )
        else:
        ...
        if (
            ...
            or condition_over_position
            or condition_in_to_out_position
        ):
            calc_amounts()
