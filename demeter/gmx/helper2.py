...
def load_gmx_v2_data(
    chain: str, gm_token_address: str, start_date: date, end_date: date, data_path: str
) -> pd.DataFrame:
    logger = logging.getLogger("Gmx v2 data")

    cache_key = CacheManager.get_cache_key(MarketTypeEnum.gmx_v2.name, start_date, end_date, chain, gm_token_address)
    ...
    day = start_date
    while day <= end_date:
        csv_path = os.path.join(data_path, f"{chain.lower()}-GmxV2-{gm_token_address}-{day.strftime('%Y-%m-%d')}.minute.csv")
