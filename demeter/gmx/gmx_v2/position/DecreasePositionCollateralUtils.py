    ...
    def processCollateral(
        ...
        collateralCache.wasSwapped, collateralCache.swapOutputAmount = (
            DecreasePositionSwapUtils.swapProfitToCollateralToken(
                params, cache.pnlToken, values.output.secondaryOutputAmount, pool_status, pool_config
            )
        )
        ...
        if values.basePnlUsd < 0:
            values, collateralCache.result = DecreasePositionCollateralUtils.payForCost(
                params, values, cache.prices, cache.collateralTokenPrice.min, -values.basePnlUsd
            )
            if collateralCache.result.remainingCostUsd > 0:
                return values, PositionFees()
                pass  # todo handleEarlyReturn
        ...
        if values.priceImpactUsd < 0:
            values, collateralCache.result = DecreasePositionCollateralUtils.payForCost(
                params, values, cache.prices, cache.collateralTokenPrice.min, -values.priceImpactUsd
            )
        ...
        if values.priceImpactDiffUsd > 0:
            values, collateralCache.result = DecreasePositionCollateralUtils.payForCost(
                params, values, cache.prices, cache.collateralTokenPrice.min, values.priceImpactDiffUsd
            )
            if collateralCache.result.remainingCostUsd > 0:
                return values, PositionFees()
                pass  # todo handleEarlyReturn
