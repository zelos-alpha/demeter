...
class TestUniLpMarketToken1Base(unittest.TestCase):
    ...
    def test_collect_fee(self):
        ...
        (new_position, base_used, quote_used, liquidity) = market._add_liquidity_by_tick(
            Decimal(1), market.market_status.data.price, market.market_status.data.closeTick - 10, market.market_status.data.closeTick + 10
        )
        TestUniLpMarketToken1Base.print_broker(broker)
    ...
    def test_collect_fee_down(self):
        ...
        (new_position, base_used, quote_used, liquidity) = market._add_liquidity_by_tick(
            Decimal(1), market.market_status.data.price, market.market_status.data.closeTick - 100, market.market_status.data.closeTick - 10
        )
        TestUniLpMarketToken1Base.print_broker(broker)
        eth_amount = 10000000000000000000
        usdc_amount = 10000000
        ...
        market.set_market_status(
            MarketStatus(
                timestamp=None,
                data=pd.Series(
                    data=[current, liquidity * 100, eth_amount, usdc_amount, price, current - 120],
                    index=["closeTick", "currentLiquidity", "inAmount0", "inAmount1", "price", "last_tick"],
                ),
            ),
            price=None,
        )
    ...
    def test_collect_fee_in_out(self):
        ...
        market.set_market_status(
            MarketStatus(
                timestamp=None,
                data=pd.Series(
                    data=[current, liquidity * 100, eth_amount, usdc_amount, price, current - 50],
                    index=["closeTick", "currentLiquidity", "inAmount0", "inAmount1", "price", "last_tick"],
                ),
            ),
            price=None,
    ...
    def test_collect_fee_up(self):
        ...
        market.set_market_status(
            MarketStatus(
                ...
                data=pd.Series(
                    data=[
                        market.market_status.data.closeTick - 120,
                        liquidity * 100,
                        eth_amount,
                        usdc_amount,
                        price,
            ...
            MarketStatus(
                ...
                data=pd.Series(
                    data=[
                        ...
                        usdc_amount,
                        price,
                        market.market_status.data.closeTick - 110,
                    ],
                    index=["closeTick", "currentLiquidity", "inAmount0", "inAmount1", "price", "last_tick"],
                ),
            ),
    ...
    def test_buy(self):
        ...
        TestUniLpMarketToken1Base.print_broker(broker)
        self.assertEqual(
            broker.assets[self.usdc].balance, token1_before - market.market_status.data.price * Decimal(0.5) * (1 + market.pool_info.fee_rate)
        )
    ...
    def test_sell(self):
        ...
        TestUniLpMarketToken1Base.print_broker(broker)
        self.assertEqual(
            broker.assets[market.token1].balance, token1_before + market.market_status.data.price * Decimal(1) * (1 - market.pool_info.fee_rate)
        )
